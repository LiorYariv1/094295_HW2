...
class Episode(object):
    ...
    def calc_qvals(self, gamma: float) -> List[float]:
        ...
        for i in range(2,len(self.experiences)+1):
            cur_reward = self.experiences[-i].reward
            qvals[-i]= (cur_reward+gamma*qvals[-i+1])
...
class TrainBatch(object):
    ...
    def from_episodes(cls, episodes: Iterable[Episode], gamma=0.999):
        ...
        for i,episode in enumerate(episodes):
            episode_qvals = episode.calc_qvals(gamma)
            episode_states, episode_actions = [], []
            for exp in episode.experiences:
                episode_states.append(exp.state.clone().detach())
                episode_actions.append(exp.action.clone().detach())
            if i==0:
                states = torch.stack(episode_states)
                actions = torch.tensor(episode_actions)
                total_reward = torch.tensor(episode.total_reward).unsqueeze(0)
                qvals = torch.tensor(episode_qvals)
            else:
                states = torch.cat((states, torch.stack(episode_states)))
                actions = torch.cat((actions,torch.tensor(episode_actions)))
                total_reward = torch.cat((total_reward,torch.tensor(episode.total_reward).unsqueeze(0)))
                qvals = torch.cat((qvals, torch.tensor(episode_qvals)))
        train_batch = cls(states,actions,qvals,total_reward)
        # ========================
        return train_batch
...
class TrainBatchDataset(torch.utils.data.IterableDataset):
    ...
    def episode_batch_generator(self) -> Iterator[Tuple[Episode]]:
        ...
        while True:
            ...
            while not is_done:
                step_exp = agent.step()
                episode_experiences.append(step_exp)
                episode_reward += step_exp.reward
                is_done = step_exp.is_done
