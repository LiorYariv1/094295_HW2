...
class PolicyNet(nn.Module):
    ...
    def forward(self, x):
        # TODO: Implement a simple neural net to approximate the policy.
        # ====== YOUR CODE: ======
        action_scores = self.simple_net(x)
...
class PolicyAgent(object):
    ...
    def monitor_episode(
        ...
        with gym.wrappers.Monitor(
            ...
            # ====== YOUR CODE: ======
            policy_agent = cls(env,p_net,device)            
            episode_done = False
            while not episode_done:
                _,_,cur_reward, episode_done = policy_agent.step()
                reward += cur_reward
                n_steps+=1
...
class VanillaPolicyGradientLoss(nn.Module):
    ...
    def _policy_loss(self, batch, action_scores, policy_weight):
        ...
        # ====== YOUR CODE: ======
        actions_probs = torch.nn.functional.log_softmax(action_scores,dim=1)
        actions_values = torch.gather(actions_probs, 1, batch.actions.unsqueeze(1)).squeeze()
        loss_p = -1*torch.mean(policy_weight*actions_values, dim=0)
        # log_proba = torch.log_softmax(action_scores,1)
        # log_proba_action = torch.gather(log_proba,1,batch.actions.to(torch.int64).unsqueeze(1))
        # loss_p = -torch.mean(log_proba_action*policy_weight)
        # raise NotImplementedError()
        # ========================
        return loss_p
...
class ActionEntropyLoss(nn.Module):
    ...
    def calc_max_entropy(n_actions):
        ...
        # ====== YOUR CODE: ======
        max_entropy = -1*np.log(1/n_actions)
        # raise NotImplementedError()
        # ========================
        return max_entropy
    ...
    def forward(self, batch: TrainBatch, action_scores, **kw):
        ...
        # ====== YOUR CODE: ======
        actions_probs = torch.nn.functional.softmax(action_scores,dim=1)
        actions_log_probs = torch.nn.functional.log_softmax(action_scores, dim=1)
        loss_per_exp = torch.sum(actions_probs*actions_log_probs, dim=1)
        loss_e = (1*torch.mean(loss_per_exp,dim=0))/self.max_entropy
        # raise NotImplementedError()
        # ========================
...
class PolicyTrainer(object):
    ...
    def train_batch(self, batch: TrainBatch):
        ...
        for loss_model in self.loss_functions:
            loss, dict_loss= loss_model(batch,action_scores)
